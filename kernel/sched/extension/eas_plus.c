...
#ifdef OPLUS_FEATURE_UIFIRST
	if (sysctl_uifirst_enabled && sysctl_launcher_boost_enabled && task_has_rt_policy(p) &&
		list_empty(&cpu_rq(new_cpu)->ux_thread_list) && !hinted_cpu_prefer(task_prefer) && !cpu_isolated(new_cpu))
#else
...
#ifdef OPLUS_FEATURE_UIFIRST
		if (sysctl_uifirst_enabled && sysctl_launcher_boost_enabled && task_has_rt_policy(p) &&
			list_empty(&cpu_rq(new_cpu)->ux_thread_list) && cpumask_test_cpu(new_cpu, &domain->possible_cpus) && !cpu_isolated(new_cpu))
#else
...
#ifdef OPLUS_FEATURE_UIFIRST
			if (sysctl_uifirst_enabled && sysctl_launcher_boost_enabled && task_has_rt_policy(p) &&
				!list_empty(&cpu_rq(iter_cpu)->ux_thread_list))
					continue;
#endif /* OPLUS_FEATURE_UIFIRST */
