...
void adsp_aed(enum adsp_excep_id type, enum adsp_core_id id)
{
	...
	/* Liang.Wang@MULTIMEDIA.AUDIODRIVER.ADSP, 2020/05/18, modify log level for adsp exception */
	pr_info("%s", detail);
#else
	pr_debug("%s", detail);
#endif
	...
	//Zhao.Pan@MULTIMEDIA.AUDIODRIVER.ADSP, 2020/07/17,add for adsp patch
