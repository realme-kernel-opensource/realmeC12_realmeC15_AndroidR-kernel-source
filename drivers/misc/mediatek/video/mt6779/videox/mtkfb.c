...
/*
 *JianBin.Zhang@PSW.MM.Display.LCD.Stability, 2019/01/21,
...
/*Jian.Zhou@PSW.MM.Display.LCD, 2020/08/26, add panelid & sau*/
...
int mtkfb_aod_mode_switch(enum mtkfb_aod_power_mode aod_pm)
{
	...
	if (aod_pm == MTKFB_AOD_DOZE_SUSPEND) {
		...
		#ifdef OPLUS_BUG_STABILITY
		/*
		*JianBin.Zhang@PSW.MM.Display.LCD.Stability, 2020/05/26,
...
static int mtkfb_probe(struct platform_device *pdev)
{
	...
	init_state = 0;
	#ifdef OPLUS_BUG_STABILITY
	/* zhangjianbin@MM.Display.LCD.Machine, 2020/3/30, modify for oppo lcd  */
	...
	/*Jian.Zhou@PSW.MM.Display.LCD, 2020/08/26, add panelid & sau read node*/
...
static int mtkfb_remove(struct platform_device *pdev)
{
	...
	#ifdef OPLUS_FEATURE_MIPICLKCHANGE
	/*
	 * Jianbin.ZhangGuo@PSW.MM.Display.LCD.Stability, 2019/01/21,
	 * add for mipi clk change
	 */
