...
#ifdef OPLUS_ARCH_EXTENDS
/* JianBin.Zhang@PSW.MM.Display.LCD.Stability, 2020/05/13,* add for display decouping*/
...
/* JianBin.Zhang@PSW.MM.Display.LCD.Stability, 2020/07/13,* add for display slience mode*/
...
/** Jianbin.Zhang@PSW.MM.Display.LCD.Stability, 2019/11/07, * modify for DC */
...
bool primary_is_aod_supported(void)
{
	...
	/* YongPeng.Yi@PSW.MM.Display.LCD.Stability, 2019/11/25, modify for ramless aod */
	/*
...
lcm_corner_out:
	...
	if (primary_display_is_video_mode()) {
		//#ifndef OPLUS_FEATURE_RAMLESS_AOD
		//YongPeng.Yi@PSW.MM.Display.LCD.Stability, 2020/01/15, modify for ramless aod
		//if (disp_helper_get_option(DISP_OPT_SWITCH_DST_MODE))
...
int primary_display_suspend(void)
{
		...
		*/
		if (primary_display_get_lcm_power_state_nolock() == LCM_ON_LOW_POWER) {
			if (oplus_display_fppress_support && !doze_rec_fpd && !ds_rec_fpd) {
				//#ifndef OPLUS_FEATURE_RAMLESS_AOD
				/* YongPeng.Yi@PSW.MM.Display.LCD.Stability, 2020/01/15, modify for ramless aod hbm */
	...
	//#ifdef VENDOR_EDIT
	/*
	* zhangjianbin@PSW.MM.Display.LCD.Stability, 2020/03/23,
...
int primary_display_lcm_power_on_state(int alive)
{
	...
	if (primary_display_get_power_mode_nolock() == DOZE) {
		...
		if (primary_display_get_lcm_power_state_nolock() !=
			LCM_ON_LOW_POWER) {
#ifndef VENDOR_EDIT
/** JianBin.Zhang@PSW.MM.Display.LCD.Stability, 2020/04/17,* modify for support aod state.*/
			if (pgc->plcm->drv->aod)
				disp_lcm_aod(pgc->plcm, 1);
			else if (!alive)
			...
			if (pgc->plcm->drv->aod) {
				if (primary_display_get_lcm_power_state_nolock() == LCM_ON) {
					...
					//#ifdef OPLUS_FEATURE_RAMLESS_AOD
					/* YongPeng.Yi@PSW.MM.Display.LCD.Stability, 2020/01/15, add for ramless aod hbm */
					if ((oplus_display_aod_ramless_support && rlta_enable)) {
						if (oplus_display_fppress_support && !doze_rec_fpd && !ds_rec_fpd) {
							if (oplus_display_aod_ramless_support) {
								if (primary_display_is_video_mode()) {
...
int primary_display_resume(void)
{
	...
	*/
...

	{
		...
		//#ifndef OPLUS_FEATURE_RAMLESS_AOD
		/* YongPeng.Yi@PSW.MM.Display.LCD.Stability, 2020/02/07, modify for 19357 ramless aod */
		/*
...
int primary_display_aod_backlight(int level)
{
	...
	/** Jianbin.Zhang@PSW.MM.Display.LCD.Stability, 2020/05/09,* add for get aod power mode*/
	//_primary_path_lock(__func__);
	//#else
...
int primary_display_frame_cfg(struct disp_frame_cfg_t *cfg)
{
	...
	#ifdef OPLUS_BUG_STABILITY
	/* Zhijun.Ye@PSW.MM.Display.LCD.Stability, 2020/04/01,
	...
	#ifdef OPLUS_FEATURE_ONSCREENFINGERPRINT
	/** Jianbin.Zhang@PSW.MM.Display.LCD.Stability, 2020/05/09, * modify for  OPLUS_FEATURE_ONSCREENFINGERPRINT*/
	if (oplus_display_fppress_support && !ds_rec_fpd && !doze_rec_fpd) {
		...
		//#ifndef OPLUS_FEATURE_RAMLESS_AOD
		/* YongPeng.Yi@PSW.MM.Display.LCD.Stability, 2020/01/29, modify for ramless aod hbm */
...
int primary_display_set_lcm_hbm(bool en)
{
	...
	if (state == -1)
		return -EINVAL;
#ifndef OPLUS_BUG_STABILITY
/* YongPeng.Yi@PSW.MM.Display.LCD.Stability, 2020/01/29, add for CLOSE slice mode hbm */
	else if (state == en)
		return 0;
#else
...
//#ifdef OPLUS_BUG_SATBILITY
/** Jianbin.Zhang@PSW.MM.Display.LCD.Feature, 2020/05/20,* add for get dimming layer hbm state*/
...
/*
 * Jianbin.Zhang@PSW.MM.Display.LCD.Stability, 2020/04/17,
...
int primary_display_ccci_mipi_callback(int en, int usrdata)
{
	...
	/** JianBin.Zhang@PSW.MM.Display.LCD.Stability, 2020/04/16,* add for skipping mipi clk change when display is suspend*/
	if (pgc->state == DISP_SLEPT) {
