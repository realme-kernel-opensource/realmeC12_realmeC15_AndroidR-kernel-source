...
//WuWeiZhong@BSP.Power.Basic, 2020/08/15, add for wakeup statics.
...
#ifdef OPLUS_FEATURE_POWERINFO_STANDBY
//WuWeiZhong@BSP.Power.Basic, 2020/11/12, add for statics clk info.
	...
	const struct wake_status *wakesta, bool suspend, const char *scenario)
{
	...
	if (!strcmp(scenario, "suspend")) {
		...
		log_size += sprintf(log_buf + log_size,
			...
			spm_26M_off_pct);
		#ifdef OPLUS_FEATURE_POWERINFO_STANDBY
		//WuWeiZhong@BSP.Power.Basic, 2020/10/29, add for print suspend time.
