...
#define PFX "riod_hi1336_camera_sensor"
		...
		do {
			*sensor_id = return_sensor_id();
			if (*sensor_id == imgsensor_info.sensor_id) {
				if (read_head_id() != 0x00310058) {
					*sensor_id = 0xFFFFFFFF;
					LOG_INF("sensor_id: %d match fail\n", *sensor_id);
					return ERROR_SENSOR_CONNECT_FAIL;
				}
				pr_err("i2c write id : 0x%x, sensor id: 0x%x\n",
							imgsensor.i2c_write_id, *sensor_id);
				if (deviceInfo_register_value == 0x00) {
					register_imgsensor_deviceinfo("Cam_r", DEVICE_VERSION_HI1336, 0x58);
...
UINT32 RIOD_HI1336_MIPI_RAW_SensorInit(struct SENSOR_FUNCTION_STRUCT **pfFunc)
{
