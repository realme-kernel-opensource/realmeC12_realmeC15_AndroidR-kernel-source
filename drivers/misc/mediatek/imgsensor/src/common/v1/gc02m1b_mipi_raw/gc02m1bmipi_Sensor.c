...
#define PFX "riod_gc02m1b_camera_sensor"
...
{
	return ((read_cmos_sensor(0xf0) << 8) | read_cmos_sensor(0xf1)) + 3 + 7;
}
			...
			*sensor_id = return_sensor_id();
			...
			if (*sensor_id == imgsensor_info.sensor_id) {
				if (deviceInfo_register_value == 0x00) {
					register_imgsensor_deviceinfo("Cam_r3", DEVICE_VERSION_GC02M1B, 0x54);
...
UINT32 RIOD_GC02M1B_MIPI_RAW_SensorInit(struct SENSOR_FUNCTION_STRUCT **pfFunc)
{
