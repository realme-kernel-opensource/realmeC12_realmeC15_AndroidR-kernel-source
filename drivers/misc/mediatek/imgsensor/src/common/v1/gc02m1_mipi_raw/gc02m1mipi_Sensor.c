...
#define PFX "riod_gc02m1_camera_sensor"
		...
		do {
			*sensor_id = (return_sensor_id());
			if (*sensor_id == imgsensor_info.sensor_id) {
				if (read_head_id() != 0x00310054) {
					*sensor_id = 0xFFFFFFFF;
					printk("sensor_id: %d match fail\n", *sensor_id);
					return ERROR_SENSOR_CONNECT_FAIL;
				}
				if (deviceInfo_register_value == 0x00) {
					register_imgsensor_deviceinfo("Cam_r2", DEVICE_VERSION_GC02M1, 0x54);
...
UINT32 RIOD_GC02M1_MIPI_RAW_SensorInit(struct SENSOR_FUNCTION_STRUCT **pfFunc)
{
