...
#define PFX "riod_OV02B10_camera_sensor"
...
static kal_uint64 read_head_id(void)
{
	...
	iReadRegI2C(pusendcmd_module, 2, (u8 *)&module_byte, 1, 0xA4);
	iReadRegI2C(pusendcmd_supplier, 2, (u8 *)&supplier_byte, 1, 0xA4);
        ...
        do {
            *sensor_id = (return_sensor_id());
            if (*sensor_id == imgsensor_info.sensor_id) {
				if (read_head_id() != 0x00310008) {
					*sensor_id = 0xFFFFFFFF;
					printk("sensor_id: %d match fail\n", *sensor_id);
					return ERROR_SENSOR_CONNECT_FAIL;
				}
				if (deviceInfo_register_value == 0x00) {
					register_imgsensor_deviceinfo("Cam_r2", DEVICE_VERSION_OV02B10, 0x08);
...
UINT32 RIOD_OV02B10_MIPI_RAW_SensorInit(struct SENSOR_FUNCTION_STRUCT **pfFunc)
{
