...
#define IMGSENSOR_MODULE_ID_QTECH       0x06
...
#ifdef OPLUS_FEATURE_CAMERA_COMMON
static inline int imgsensor_get_seriano_from_ic(MINT8 *seriano,
	struct IMGSENSOR_SENSOR *psensor)
{
	struct IMGSENSOR_SENSOR_INST  *psensor_inst = &psensor->inst;
	MUINT32 retLen = 0;
	...
	retLen = imgsensor_hw_power(&pgimgsensor->hw,
				psensor,
				psensor_inst->psensor_name,
				IMGSENSOR_HW_POWER_STATUS_ON);

	if (retLen == IMGSENSOR_RETURN_SUCCESS)
		imgsensor_sensor_feature_control(
			psensor,
...
void imgsensor_get_seriano(MINT8 *seriano, MINT32 drv_idx, struct IMGSENSOR_SENSOR *psensor)
{
	...
	if (pSensorList[drv_idx].init) {
		...
		if (psensor->pfunc) {
			if (imgsensor_get_seriano_from_ic(seriano, psensor)) {
				PK_DBG("get seriano success");
			}
		}
	}
...
static inline int adopt_CAMERA_HW_FeatureControl(void *pBuf)
{
	...
	case SENSOR_FEATURE_GET_SENSOR_SYNC_MODE:
		...
		break;
	#ifdef OPLUS_FEATURE_CAMERA_COMMON
	case SENSOR_FEATURE_GET_SERIANO:
	{
		MUINT8 seriano[17] = {0};
		MINT32 drv_idx;
		drv_idx = FeatureParaLen;
		PK_DBG("drv_idx :%d", drv_idx);
		psensor->inst.sensor_idx = pFeatureCtrl->InvokeCamera;
		imgsensor_get_seriano(seriano, drv_idx, psensor);
		memcpy(pFeaturePara, seriano, 17);
		if (copy_to_user(
					(void __user *) pFeatureCtrl->pFeaturePara,
					(void *)pFeaturePara, 17)) {
					kfree(pFeaturePara);
					PK_DBG(
						"[CAMERA_HW][pSensorRegData] ioctl copy to user failed\n");
					return -EFAULT;
