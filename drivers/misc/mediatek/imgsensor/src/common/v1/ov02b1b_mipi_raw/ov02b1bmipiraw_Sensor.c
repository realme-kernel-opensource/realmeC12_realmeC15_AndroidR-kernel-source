...
#define PFX "riod_ov02b1b_camera_sensor"
			...
			*sensor_id = ((read_cmos_sensor(0x02) << 8) | read_cmos_sensor(0x03)) + 8;
			...
			if (*sensor_id == imgsensor_info.sensor_id) {
				if (deviceInfo_register_value == 0x00) {
					register_imgsensor_deviceinfo("Cam_r3", DEVICE_VERSION_OV02B1B, 0x08);
...
UINT32 RIOD_OV02B1B_MIPI_RAW_SensorInit(struct SENSOR_FUNCTION_STRUCT **pfFunc)
{
