...
static void lcm_setbacklight_cmdq(void *handle, unsigned int level)
{
	...
	if (aod_in && !aod_state) {
		...
		/* Jianbin.Zhang@MM.Display.LCD.Stability, 2020/12/01,
		add for modify the backlight of charge animation.
		*/
		...
		if (!aod_out && mapped_level > BRIGHTNESS_OFF && aod_state) {
			/* Zhijun.Ye@MM.Display.LCD.Stability, 2020/09/17,
			 * add for fixing no backlight on homescreen after unlocking.
			 */
			aod_in = false;
			aod_out = true;
			aod_state = false;
