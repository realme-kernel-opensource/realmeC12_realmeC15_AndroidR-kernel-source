...
#Wuyue.Lei@ANDROID.STABILITY for KASAN or KMEMLEAK aging version
...
then
        ...
        delete_kernel_config CONFIG_OPPO_SVELTE
        setting_kernel_config CONFIG_RANDOMIZE_BASE n 0
...
if [[ "$OPPO_AGING_TEST" == 1 ]] && [[ "$OPPO_MEMLEAK_DETECT" != 0 ]]
then
	# enable memleak detect thread
	setting_kernel_config CONFIG_OPPO_MEMLEAK_DETECT true 1
	setting_kernel_config CONFIG_MEMLEAK_DETECT_THREAD y 0
elif [[ "$OPPO_MEMLEAK_DETECT" == 1 ]]
then
	# enable memleak detect thread
	setting_kernel_config CONFIG_OPPO_MEMLEAK_DETECT true 1
	setting_kernel_config CONFIG_MEMLEAK_DETECT_THREAD y 0
elif [[ "$OPPO_MEMLEAK_DETECT" == 2 ]]
then
	# enable memleak detect thread
	setting_kernel_config CONFIG_OPPO_MEMLEAK_DETECT true 1
	setting_kernel_config CONFIG_MEMLEAK_DETECT_THREAD y 0
...
echo  "===OPPO_MEMLEAK_DETECT = $OPPO_MEMLEAK_DETECT"
