...
#ifdef VENDOR_EDIT
/* Jingjing.Jiang@PSW.MM.AudioDriver.SmartPA, 2020/05/03, for load firmware by kcontrol*/
...
static int tfa98xx_mute(struct snd_soc_dai *dai, int mute, int stream)
{
	...
	if (mute) {
		...
		/* Jingjing.Jiang@PSW.MM.AudioDriver.SmartPA, 2020/03/03,
		 * reduce waiting time for kernel Underflow caused by AudioFlinger */
			if (tfa_dual_start_cnt == 2) {
