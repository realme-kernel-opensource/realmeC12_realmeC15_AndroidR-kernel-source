	...
	if (HAS_CAP(accdet->data->caps, ACCDET_PMIC_EINT0)) {
		if (eintID == PMIC_EINT0) {
			...
			ret = queue_work(accdet->eint_workqueue,
					&accdet->eint_work);
/* Xiaofei.Gong@bsp.tp 2020.1117 add for notifying touchpanel switch headset mode when detected plug in/out start */
			if (accdet->cur_eint_state == EINT_PLUG_IN) {
...
/* Xiaofei.Gong@bsp.tp 2020.1117 add for notifying touchpanel switch headset mode when detected plug in/out end */
		} else
